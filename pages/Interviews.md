---
title: Interviews
layout: page
permalink: /Interviews.html
---
<div class="row">
  <div class="column">
    <div class="card">
      <img src="https://strikerz45.github.io/Coin-op/objects/Portrait_3.jpg" alt="doc" style="width:98%">
      <div class="container">
        <h2>Doc Mack</h2>
        <p class="title">Owner &amp; Founder</p>
        <p>Some text that describes me lorem ipsum ipsum lorem.</p>
        <a href="https://github.com/strikerZ45/Coin-op/blob/main/pages/Doc_Mack,html">
        <p><button class="button">More about</button></p>
        </a>
      </div>
    </div>
  </div>
 <div class="column">
    <div class="card">
      <img src="https://strikerz45.github.io/Coin-op/objects/Sean_Davis_Portrait.jpg" alt="Sean" style="width:100%">
      <div class="container">
        <h2>Sean Davis</h2>
        <p class="title">Director</p>
        <p>Some text that describes me lorem ipsum ipsum lorem.</p>
        <a href="https://strikerz45.github.io/Coin-op/Lucky_Strike_Arcade.html">
        <p><button class="button">Learn more</button></p>
        </a>
      </div>
    </div>
  </div>

  <div class="column">
    <div class="card">
      <img src="https://strikerz45.github.io/Coin-op/objects/spanos01.jpg" alt="Spanos" style="width:100%">
      <div class="container">
        <h2>George Spanos</h2>
        <p class="title">Freelancer</p>
        <p>Some text that describes me lorem ipsum ipsum lorem.</p>
        <a href="https://strikerz45.github.io/Coin-op/item.html?id=georgeSpanos03">
        <p><button class="button">About</button></p>
        </a>
      </div>
    </div>
  </div>
</div>
